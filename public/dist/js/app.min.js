      ...
      resolve: {
        menuList: function (menuService, loginService) {
          return loginService.getCheckLogin().then(function () {
            return menuService.getMenuList().then(function (menu) {
              return menu;
            });
          });
        }
 ...
 * Created by Administrator on 2014-08-08.
 */
angular.module('loginRetryQueue', []).factory('loginRetryQueue', [
  '$q',
  '$log',
  '$location',
 ...
 */
angular.module('johayo.directive').directive('bookmark', function () {
  return {
    restrict: 'AE',
    templateUrl: 'bookmark/bookmark.html',
    controller: 'bookmarkController'
 ...
 */
angular.module('johayo.directive').directive('commentBox', function () {
  return {
    restrict: 'AE',
    replace: true,
 ...
 */
angular.module('johayo.directive').directive('searchBox', function () {
  return {
    restrict: 'AE',
    scope: { searchText: '=' },
  ...
  function ($window) {
    return {
      restrict: 'AE',
      template: '<div class=\'spinner\' ng-style=\'{{spinnerStyle}}\' ng-if=\'!urlImgFile.url\'>' + '<div class=\'spinner-container container1\'><div class=\'circle1\'></div><div class=\'circle2\'></div><div class=\'circle3\'></div><div class=\'circle4\'></div></div>' + '<div class=\'spinner-container container2\'><div class=\'circle1\'></div><div class=\'circle2\'></div><div class=\'circle3\'></div><div class=\'circle4\'></div></div>' + '<div class=\'spinner-container container3\'><div class=\'circle1\'></div><div class=\'circle2\'></div><div class=\'circle3\'></div><div class=\'circle4\'></div></div></div>',
      scope: { spinnerStyle: '@' }
    };
  }
]);
 ...
 * Created by 동준 on 2014-11-14.
 */
angular.module('johayo.directive').directive('johayoMenu', [
  '$window',
  function ($window) {
    return {
      restrict: 'A',
 ...
 * Created by Administrator on 2014-08-11.
 */
angular.module('johayo.filter', []).filter('limitAndJjum', [
  'limitToFilter',
...
]).filter('cutHtmlTagAndLimit', [
  'limitToFilter',
  function (limitToFilter) {
    return function (input, limit) {
      if (input) {
        var changeInput = input.replace(/(<([^>]+)>)/gi, '');
 ...
 */
angular.module('johayo.service').factory('boardService', [
  '$resource',
  '$q',
  '$http',
  function ($resource, $q, $http) {
    var boardApi = {
        list: $resource('/api/board/list/:division', { division: '@division' }, {
          'query': {
            method: 'GET',
            isArray: true
          }
 ...
 */
angular.module('johayo.service').factory('bookmarkService', [
  '$http',
  '$q',
  function ($http, $q) {
    var service = {
 ...
 */
angular.module('johayo.service').factory('commentService', [
  '$http',
  '$q',
  '$resource',
  '$location',
 ...
 */
angular.module('johayo.service').factory('errorService', [
  'ngDialog',
  'errorQueue',
  function (ngDialog, queue) {
 ...
 */
angular.module('johayo.service').factory('fileService', [
  '$resource',
  '$sce',
  '$q',
  '$upload',
  function ($resource, $sce, $q, $upload) {
    var service = {
 ...
 */
angular.module('johayo.service').factory('loginService', [
  '$http',
  '$q',
  '$rootScope',
  'loginRetryQueue',
  'ngDialog',
  function ($http, $q, $rootScope, queue, ngDialog) {
 ...
 * Created by 동준 on 2014-11-17.
 */
angular.module('johayo.service').factory('menuService', [
  '$http',
  '$q',
  function ($http, $q) {
    var service = {
 ...
 */
angular.module('johayo.service').factory('msgService', [
  '$http',
  '$q',
  'ngDialog',
  function ($http, $q, ngDialog) {
    // Login form dialog stuff
    var service = {
        openMsg: function () {
          ngDialog.open({
 ...
 * Created by 동준 on 2014-11-25.
 */
angular.module('johayo.controller').controller('boardController', [
  '$scope',
  'boardService',
  'boardList',
  '$location',
  ...
  function ($scope, boardService, boardDetail, $location, $routeParams) {
    ...
    $scope.boardConfig = [
      {
        text: '<i class="glyphicon glyphicon-pencil"></i> Edit',
        click: 'showEditor()'
      },
      {
        text: '<i class="glyphicon glyphicon-remove"></i> Delete',
        click: 'deleteBoard()'
      }
    ];
  ...
  function ($scope, boardService, menuList, fileService) {
    ...
    $scope.uploadFile = function (index) {
      ...
      fileService.fileUpload($scope.selectedFiles[index], 'board').then(function (data) {
        $scope.uploadFileList.push(data);
      }, function (err) {
        $scope.selectedFiles[index].progress = 0;
        $scope.selectedFiles[index].error = err;
      }, function (evt) {
        $scope.selectedFiles[index].progress = Math.min(100, parseInt(100 * evt.loaded / evt.total));
      });
    };
 ...
 */
angular.module('johayo.controller').controller('bookmarkController', [
  '$scope',
  'bookmarkService',
  'boardService',
  'fileService',
  function ($scope, bookmarkService, boardService, fileService) {
    ...
    $scope.getUrlImg = function (url) {
      ...
      bookmarkService.getImg(url).then(function (data) {
 ...
 * Created by 동준 on 2014-11-27.
 */
angular.module('johayo.controller').controller('commentController', [
  '$scope',
  'commentService',
  function ($scope, commentService) {
    ...
    $scope.addComment = function () {
      commentService.addComment($scope.boardDetail._id, $scope.comment.content, $scope.comment.name, $scope.comment.pw).then(function (data) {
    ...
    $scope.editComment = function (commentDetail) {
      commentService.editComment($scope.boardDetail._id, commentDetail.seq, commentDetail.content, commentDetail.pw).then(function (data) {
        console.log(data);
        $scope.closeEditor();
        $scope.boardDetail = data;
    ...
    $scope.deleteComment = function (commentSeq, pw) {
      commentService.deleteComment($scope.boardDetail._id, commentSeq, pw).then(function (data) {
        console.log(data);
        $scope.boardDetail = data;
      });
    };
    ...
    $scope.checkVal = function (val) {
      ...
      if (!!val.content) {
        checkContent = val.content.replace(/(<([^>]+)>)/gi, '') == '';
      }
 ...
 * Created by 동준 on 2014-11-14.
 */
angular.module('johayo.controller').controller('indexController', [
  '$rootScope',
  '$scope',
  'loginService',
  'errorService',
  ...
  function ($rootScope, $scope, loginService, errorService, msgService) {
    ...
    $scope.adminMenu = [
      {
        text: '<i class="glyphicon glyphicon-ok"></i> Menu',
        href: '/#/admin/menu'
      },
      {
        text: '<i class="glyphicon glyphicon-pencil"></i> Write board',
        href: '/#/admin/board'
      },
      { 'divider': true },
      {
        text: '<i class="glyphicon glyphicon-log-out"></i> Logout',
        click: 'logout()'
      }
    ];
    /**
 ...
 * Created by 동준 on 2014-11-17.
 */
angular.module('johayo.controller').controller('loginController', [
  '$scope',
  'loginService',
  'ngDialog',
  '$alert',
 ...
 * Created by 동준 on 2014-11-14.
 */
angular.module('johayo.controller').controller('mainController', [
  '$scope',
  'boardList',
  function ($scope, boardList) {
    ...
    $scope.helper = [
      {
        text: '<i class="glyphicon glyphicon-thumbs-up"></i> \ubc15\ub3d9\uc7ac',
        click: 'helperClick()'
      },
      {
        text: '<i class="glyphicon glyphicon-thumbs-up"></i> \uc720\uc790\uc131',
        click: 'helperClick()'
      },
      {
        text: '<i class="glyphicon glyphicon-thumbs-up"></i> angular \uc2a4\ud130\ub514\uadf8\ub8f9',
        click: 'helperClick()'
      },
      { 'divider': true },
      {
        text: '\ub4f1\ub4f1 \uc5ec\ub7ec \ubaa8\ub4e0 \ubd84\ub4e4..<i class="glyphicon glyphicon-thumbs-up"></i><i class="glyphicon glyphicon-thumbs-up"></i>',
        click: 'helperClick()'
      }
    ];
  }
 ...
 * Created by 동준 on 2014-11-14.
 */
angular.module('johayo.controller').controller('adminMenuController', [
  '$rootScope',
  '$scope',
  'menuService',
  'menuList',
  ...
  function ($rootScope, $scope, menuService, $location) {
    ...
    $scope.isActive = function (name) {
      return { 'active': $scope.checkActive(name) };
    };
    ...
    $scope.isSubActive = function (name) {
      return { 'active': $scope.activeSubMenu == name };
    };
    /* class를 가지고 온다. */
 ...
 * Created by 동준 on 2014-12-11.
 */
angular.module('johayo.controller').controller('msgController', [
  '$scope',
  'msgService',
  '$alert',
  function ($scope, msgService, $alert) {
    ...
    $scope.sendMsg = function () {
      ...
      msgService.sendMsg($scope.msg).then(function () {
        alert = $alert({
          title: 'DongJun Kwon\uc5d0\uac8c \uba54\uc138\uc9c0(\uc774\uba54\uc77c,\ud154\ub808\uadf8\ub7a8) \ubcf4\ub0b4\uae30 \uc131\uacf5\ud588\uc2b5\ub2c8\ub2e4.',
          type: 'success',
          show: true,
          container: '#alerts-container'
        });
